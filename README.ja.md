...
付箋、クリップ、テープと紙片で、日常の写真を一頁の視覚日記にする。
...
日常写真を編集イラストや視覚日記にしたいときに。上半分は識別できる原写真を残し、下半分はシールの山でも手帳テンプレートでもない。意味と情緒を読んだうえで、オフィスの品物を隠喩にし、手描きコラージュで留まるべき一瞬をつくる。

選択的記憶、物件の物語、手描きコラージュ、大きな余白。抑制され、鋭く、詩的で現代的。逐物の複製、画面の埋め尽くし、素材の羅列、商業手帳テンプレ、カートゥーン、安価なコラージュは避ける。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-225` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-225 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-225-prompt.en.md)／[中国語](references/xxd-panel-225-prompt.zh-CN.md)を参照してください。
...
- `design-only`：全画面を Panel 225 のデザイン翻訳にし、写真は見えない参照にします。
