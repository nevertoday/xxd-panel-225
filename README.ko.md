...
메모지, 클립, 테이프와 종이 조각으로 평범한 사진을 한 페이지의 시각 일기로 쓴다.
...
일상 사진을 편집 일러스트나 시각 일기로 만들고 싶을 때. 상단은 알아볼 수 있는 원사진을 남기고, 하단은 스티커 더미나 다이어리 템플릿이 아니다. 의미와 정서를 읽은 뒤 사무용품으로 환유하고, 손그림 콜라주로 머물 만한 순간을 만든다.

선택적 기억, 사물 서사, 손그림 콜라주, 넓은 여백. 절제되고 예리하며 시적이고 동시대적. 그대로의 복제, 화면을 채우기, 소재 나열, 상업 다이어리 템플릿, 카툰, 값싼 콜라주는 피한다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-225`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-225 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-225-prompt.en.md)와 [중국어](references/xxd-panel-225-prompt.zh-CN.md)를 확인하세요.
...
- `design-only`: 전체 캔버스에 Panel 225의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
